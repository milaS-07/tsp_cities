...
def genetski_algoritam_prepravljen(g):
    broj = len(g.nodes)
    ...
    sortiranaPopulacija = sorted(populacija, key=lambda x: x[1])
    najbolja_jedinka = sortiranaPopulacija[0]
    
    najbolja_ruta = [0] + najbolja_jedinka[0]
    najbolja_duzina = najbolja_jedinka[1]

    return najbolja_ruta, najbolja_duzina


# --- GLAVNI EKSPERIMENTALNI KOD ---

N = 3              # broj čvorova
ALGORITAM = "ga"   # izbor: "bf", "nn", "ga", "sva_tri"
...
if ALGORITAM in ["ga", "sva_tri"]:
    ...
    for i in range(BROJ_GA_RUNOVA):
        t0 = time.perf_counter()
        ga_ruta, ga_duzina = genetski_algoritam_prepravljen(g)
